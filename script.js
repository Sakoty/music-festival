...
previewSound.addEventListener("click", async () => {
  ...
  // ユーザー操作内で AudioContext を確実に resume（iOS対策）
  try {
    await ensureAudioContext();
  } catch (e) {
  ...
  // まず WebAudio で再生を試みる（デコード済みバッファがあれば即再生）
  try {
    const buf = await loadBuffer(url);
    if (buf) {
      const src = audioCtx.createBufferSource();
  ...
  // WebAudio がダメなら HTMLAudio にフォールバック（iOS向け属性セット）
  try {
    const a = new Audio();
    a.src = url;
    a.volume = volume;
    // iOS の中断を避けるため inline 再生属性を付与
    a.setAttribute("playsinline", "");
    a.setAttribute("webkit-playsinline", "");
    // 一時的に DOM に追加して確実に再生する（自動で消す）
    ...
    console.warn("fallback HTMLAudio failed:", e);
  }

  // 最終手段：ユーザーに原因を知らせる
  statusEl.textContent = "状態：プレビュー失敗";
  alert("プレビュー再生に失敗しました。\n・CORSでブロックされている可能性\n・URLが間違っている可能性\n・端末の音量やサイレントスイッチを確認してください");
});
